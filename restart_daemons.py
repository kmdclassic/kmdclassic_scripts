#!/usr/bin/env python3
"""
Script to gracefully stop and optionally restart all kmdclassicd/kmdclassic-qt processes.
...
def find_komodo_processes() -> List[DaemonInfo]:
    """
    Find all running kmdclassicd or kmdclassic-qt processes and extract their information.
    ...
    try:
        ...
        for line in result.stdout.split('\n'):
            # Match kmdclassicd or kmdclassic-qt processes
            if 'kmdclassicd' in line or 'kmdclassic-qt' in line:
...
def determine_config_path(daemon: DaemonInfo) -> str:
    ...
    if daemon.datadir:
        ...
        base_dir = os.path.join(home, '.kmdclassic', daemon.ac_name)
    else:
        base_dir = os.path.join(home, '.kmdclassic')
    
    # Determine config filename
    if daemon.ac_name:
        config_file = f'{daemon.ac_name}.conf'
    else:
        config_file = 'kmdclassic.conf'
...
def read_config(config_path: str) -> Dict[str, Optional[str]]:
    ...
    try:
        # Read config file line by line (not using ConfigParser as kmdclassic.conf format is simple)
...
def is_process_running(pid: int) -> bool:
    ...
    Also verifies it's still a kmdclassicd/kmdclassic-qt process (not a reused PID).
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            if 'kmdclassicd' in comm or 'kmdclassic-qt' in comm:
                return True
...
def main():
    ...
    print("\n[Step 1] Finding kmdclassicd/kmdclassic-qt processes...")
    daemons = find_komodo_processes()
    
    if not daemons:
        print("No kmdclassicd or kmdclassic-qt processes found.")
        return
